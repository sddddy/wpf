...
class SpatialTemporalConv(nn.Module):
    def __init__(self, seq_len, capacity, in_var, output_dim, dropout=0.1):
        ...
        self.gcn = pyg_nn.SAGEConv(self.seq_len, self.seq_len)
    ...
    def forward(self, src, graph, feature_graph):
        ...
        src_one = src.permute(0, 3, 1, 2).contiguous().view(bz * in_var, capacity,seq_len)
        ...
        spatial_fea = out.view(bz, in_var, self.capacity, self.seq_len)
        spatial_fea = spatial_fea.permute(0, 2, 3, 1)
...
class Model(nn.Module):
    ...
    def forward(self, batch_x, time_x=None, time_y=None, graph=None, feature_graph=None):
        ...
        mean = means[:, 0, -1].view(bz, capacity, 1)
        std = stdev[:, 0, -1].view(bz, capacity, 1)
