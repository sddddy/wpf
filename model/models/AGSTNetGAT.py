...
class SpatialTemporalConv(nn.Module):
    def __init__(self, seq_len, capacity, in_var, output_dim, dropout=0.1):
        ...
        # 使用 GATConv 替代 SAGEConv
        self.gcn = GATConv(in_channels=self.in_var, out_channels=self.in_var, heads=1, concat=False)
        self.feature_gcn = GATConv(in_channels=self.seq_len, out_channels=self.seq_len, heads=1, concat=False)
    ...
    def forward(self, src, graph, feature_graph):
        ...
        batch = Batch.from_data_list([
            Data(x=src_one[i], edge_index=edge_index) for i in range(src_one.shape[0])
        ])
