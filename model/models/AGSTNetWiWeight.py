...
class SpatialTemporalConv(nn.Module):
    def __init__(self, seq_len, capacity, in_var, output_dim, dropout=0.1):
        ...
        # 用于边权重的映射网络
        edge_nn_spatial = nn.Sequential(
            nn.Linear(1, 32),
            nn.ReLU(),
            nn.Linear(32, in_var * in_var)
        )
        self.gcn = pyg_nn.NNConv(in_channels=in_var, out_channels=in_var, nn=edge_nn_spatial, aggr='mean')

        edge_nn_feature = nn.Sequential(
            nn.Linear(1, 32),
            nn.ReLU(),
            nn.Linear(32, seq_len * seq_len)
        )
        self.feature_gcn = pyg_nn.NNConv(in_channels=seq_len, out_channels=seq_len, nn=edge_nn_feature, aggr='mean')

        self.dropout = nn.Dropout(dropout)

        self.conv1 = nn.Conv1d(in_channels=self.in_var, out_channels=self.in_var, kernel_size=3, padding=1, bias=False)
        self.conv2 = nn.Conv1d(in_channels=self.in_var, out_channels=self.in_var, kernel_size=5, padding=2, bias=False)
    ...
    def forward(self, src, graph, feature_graph):
        ...
        # --------- Spatial Graph ---------
        src_one = src.permute(0, 2, 1, 3).contiguous().view(bz * seq_len, capacity, in_var)

        edge_index = graph.edge_index
        edge_attr = graph.edge_attr.view(-1, 1)  # Ensure shape: [E, 1]

        batch = Batch.from_data_list([
            Data(x=src_one[i], edge_index=edge_index, edge_attr=edge_attr)
            for i in range(src_one.shape[0])
        ])

        out = self.gcn(batch.x, batch.edge_index, batch.edge_attr)
        spatial_fea = out.view(bz, seq_len, capacity, in_var).permute(0, 2, 1, 3)

        # --------- Feature Graph ---------
        src_two = src.permute(0, 1, 3, 2).contiguous().view(bz * capacity, in_var, seq_len)

        feature_edge_index = feature_graph.edge_index
        feature_edge_attr = feature_graph.edge_attr.view(-1, 1)

        feature_batch = Batch.from_data_list([
            Data(x=src_two[i], edge_index=feature_edge_index, edge_attr=feature_edge_attr)
            for i in range(src_two.shape[0])
        ])

        feature_out = self.feature_gcn(feature_batch.x, feature_batch.edge_index, feature_batch.edge_attr)
        fea_fea = feature_out.view(bz, capacity, in_var, seq_len).permute(0, 1, 3, 2)

        # --------- Temporal Convolution ---------
        src_three = src.reshape(bz * capacity, seq_len, in_var)
        conv1_out = self.conv1(src_three.transpose(1, 2))
        conv2_out = self.conv2(src_three.transpose(1, 2))
        temporal_fea = (conv1_out + conv2_out).transpose(1, 2).reshape(bz, capacity, seq_len, in_var)
