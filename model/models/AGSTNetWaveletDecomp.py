...
class WaveletDecomp(nn.Module):
    ...
    def forward(self, x):
        ...
        for b in range(B):
            for d in range(D):
                signal = x[b, :, d].detach().cpu().numpy()
                coeffs = pywt.wavedec(signal, self.wavelet, level=self.level)
                approx = pywt.waverec([coeffs[0]] + [None] * self.level, self.wavelet)
                approx = torch.tensor(approx[:L], dtype=x.dtype)
                seasonal[b, :, d] = x[b, :, d] - approx.to(x.device)
                trend[b, :, d] = approx.to(x.device)
...
class EncoderLayer(nn.Module):
    def __init__(self,
        ...
        self.decomp = WaveletDecomp(wavelet='db4', level=2)
